...
public class UserGateway implements UserPersistencePort {
    ...
    public Mono<User> findById(Long id) {
        return userRepository.findById(id).map(userPersistenceMapper::toSUser).switchIfEmpty(Mono.empty());
    }
    ...
    public Mono<User> save(User user) {
        return userRepository.save(userPersistenceMapper.toUserEntity(user)).map(userPersistenceMapper::toSUser);
    }

    @Override
    public void deleteById(Long id) {
        userRepository.findById(id).hasElement().doOnNext(monoEmpty -> {
            if (!monoEmpty) {
